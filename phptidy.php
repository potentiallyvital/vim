...
$replace_tabs_with_indentchar = true;
...
/* list of tokens where a space will be added before and after */
...
if ($command=="replace") {
	...
	if ($GLOBALS['keep_backup'] && $cache != $cache_orig ) {
...
function phptidy($source) {
	...
	if ($GLOBALS['replace_tabs_with_indentchar']) replace_tabs_with_indentchar($tokens);
	...
	if ($GLOBALS['fix_operator_space']) fix_operator_space($tokens);
//print_tokens($tokens);
//exit;
...
/**
 * Replaces tabs with  the indent char, which can be "\t" or any number of spaces
 *
 * @param array   $tokens (reference)
 */
function replace_tabs_with_indentchar(&$tokens) {
	global $indent_char;
	foreach ( $tokens as &$token ) {
		if ( is_array($token) && $token[0]==T_WHITESPACE) {
			$token[1]= str_replace("\t", $indent_char, $token[1]);
		}
	}
}
...
function fix_separation_whitespace(&$tokens) {
	...
	foreach ( $tokens as $key => &$token ) {
		if (is_string($token)) {
			...
			if (in_array($token[0],$interesting_controls)){
...
function is_whitespace(&$tokens,$key){
	if (is_string($tokens[$key])) return false;
	return $tokens[$key][0]==T_WHITESPACE;
}

/**
 * Adds a space before and after the operator
 *
 * @param array   $tokens (reference)
 */
function fix_operator_space(&$tokens) {
	global $operators;
	foreach ( $tokens as $key => &$token ) {
		if (!in_array($token,$operators)) continue;

		// If the previous token is not white space add one
		if ($key>0 && !is_whitespace($tokens,$key-1)){
			if (is_string($token)){
				$token=" ".$token;
			}else{
				$token[1]=" ".$token[1];
